...
public class CRUDProductoController {
	...
	public String index(Model model) {
		...
		try {
			...
			List<Producto[]> listaC=rt.getForObject(URL+"/producto/listar", List.class);
			List<Marca[]> listaDist=rt.getForObject(URL+"/marca/listar", List.class);

			model.addAttribute("productos",listaC);
			model.addAttribute("marcas",listaDist);
			model.addAttribute("producto", new Producto());
		} catch (Exception e) {
			e.printStackTrace();
		}
		
		return "empleado/CRUDProducto";
	}
	
	@RequestMapping("/registrar")
	public String registrar(@ModelAttribute Producto bean, RedirectAttributes redirect) {
		try {
			RestTemplate rt=new RestTemplate();
			Gson gson=new Gson();
		...
		return "redirect:/CRUDproducto/CRUDproducto";
